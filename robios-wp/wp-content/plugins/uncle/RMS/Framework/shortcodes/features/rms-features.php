...
function features($atts)
{
    ...
    $fa = '';
        ...
        if(have_posts())
        {
            ...
            {
                the_post();
                $icon = get_post_meta(get_the_ID(), 'featuresicon', TRUE);
                if($icon != '')
                {
                    $ic = $icon;
                }
        ...
        {
            $fa .= '<div class="container"><div class="row"><div class="col-lg-12"><h1 class="common_heading">Please Insert Some Features First.</h1></div></div></div>';
        }
        wp_reset_query();
        return $fa;
